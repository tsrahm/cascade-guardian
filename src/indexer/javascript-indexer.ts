...
export class JavaScriptIndexer {
  private supportedExtensions = ['.js', '.jsx', '.mjs', '.cjs'];
  ...
  async indexDirectory(directoryPath: string): Promise<{
    ...
    try {
      ...
      const results = {
        functions: [] as JavaScriptFunction[],
        classes: [] as JavaScriptClass[],
        modules: [] as JavaScriptModule[],
        stats: {
          files_processed: 0,
      ...
      for (const filePath of jsFiles) {
        try {
          ...
          logger.error(LogCategory.INDEXING, `Failed to index file ${filePath}`, error as Error);
        }
      }
   ...
   */
  private findJavaScriptFiles(directoryPath: string): string[] {
    const files: string[] = [];
   ...
   */
  private detectModuleType(content: string, filePath: string): 'es6' | 'commonjs' | 'umd' {
    ...
    if (hasUMD) return 'umd';
    if (hasES6Imports) return 'es6';
    if (hasCommonJS) return 'commonjs';
    
    // Default based on file extension
    if (filePath.endsWith('.mjs')) return 'es6';
   ...
   */
  private extractFunctions(content: string, lines: string[], filePath: string, moduleType: string): JavaScriptFunction[] {
    const functions: JavaScriptFunction[] = [];
    ...
    const patterns = [
      // ES6 function declarations
      {
        ...
        isExported: (match: string) => match.includes('export'),
        isAsync: (match: string) => match.includes('async')
      },
      // ES6 arrow functions
      {
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.regex.exec(content)) !== null) {
        ...
        const functionInfo: JavaScriptFunction = {
          name: functionName,
          type: pattern.type,
          file_path: filePath,
          ...
          module_type: moduleType as 'es6' | 'commonjs' | 'umd'
        };
   ...
   */
  private extractClasses(content: string, lines: string[], filePath: string, moduleType: string): JavaScriptClass[] {
    const classes: JavaScriptClass[] = [];
    ...
    for (const pattern of classPatterns) {
      ...
      while ((match = pattern.regex.exec(content)) !== null) {
        ...
        const classInfo: JavaScriptClass = {
          name: className,
          file_path: filePath,
          line_number: lineNumber,
          tier,
   ...
   */
  private extractClassMethods(content: string, className: string, startLine: number): string[] {
   ...
   * Extract module information
   */
  private extractModuleInfo(content: string, filePath: string, moduleType: string): JavaScriptModule {
    ...
    return {
      type: moduleType as 'es6' | 'commonjs' | 'umd',
      imports,
      exports,
      file_path: filePath
    };
   ...
   */
  private extractInlineComments(lines: string[], lineNumber: number): string {
   ...
   */
  private calculateTier(jsDocInfo: any): number {
