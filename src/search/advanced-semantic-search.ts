...
export class AdvancedSemanticSearch {
  ...
  private embeddingCache = getEmbeddingCache(100); // 100MB cache
   ...
   */
  private async semanticSearch(options: AdvancedSearchOptions): Promise<SearchResult[]> {
    ...
    if (options.tags && options.tags.length > 0) {
      results = results.filter(func => 
        options.tags!.some(tag => func.tags && func.tags.includes(tag))
      );
    }
   ...
   */
  private calculateAdvancedSimilarity(queryEmbedding: Float32Array, docEmbedding: Float32Array): number {
    // Cosine similarity
   ...
   */
  private applyContextualEnhancements(results: SearchResult[], options: AdvancedSearchOptions, weights: SearchWeights): SearchResult[] {
   ...
   */
  private rankAndFilter(results: SearchResult[], options: AdvancedSearchOptions): SearchResult[] {
   ...
   */
  private calculateKeywordScore(row: any, options: AdvancedSearchOptions): number {
   ...
   */
  private calculateConfidence(result: any, matchType: string, semanticScore?: number): number {
   ...
   */
  async contextualSearch(query: string, context: string): Promise<string[]> {
   ...
   */
  private async findRelatedTerms(queryEmbedding: Float32Array, contextEmbedding: Float32Array): Promise<string[]> {
    const stmt = this.db.prepare(`
    ...
    return similarities
      ...
      .slice(0, 10)
      .flatMap(func => {
        const terms: string[] = [func.name];
        if (func.what) terms.push(...func.what.split(' ').filter((word: string) => word.length > 3));
        if (func.how) terms.push(...func.how.split(' ').filter((word: string) => word.length > 3));
        if (func.tags) terms.push(...func.tags.split(',').map((tag: string) => tag.trim()));
        return terms;
      })
  ...
  getSearchStats(): {
    ...
    return {
      total_functions: (totalStmt.get() as any).count,
      indexed_functions: (indexedStmt.get() as any).count,
