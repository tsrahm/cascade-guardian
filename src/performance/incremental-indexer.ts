...
import { openDatabase, setMetadata } from '../database/db.js';
...
export class IncrementalIndexer {
  ...
  private embeddingCache = getEmbeddingCache(50); // 50MB cache for embeddings
   ...
   */
  async updateIndex(): Promise<IndexingStats> {
   ...
   */
  private scanCurrentFiles(): FileMetadata[] {
  ...
  private scanDirectory(dirPath: string): FileMetadata[] {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (this.config.excludeDirectories.includes(entry.name)) continue;
   ...
   */
  private findDeletedFiles(currentFiles: FileMetadata[]): string[] {
   ...
   */
  private findNewAndChangedFiles(currentFiles: FileMetadata[]): {
   ...
   */
  private async removeDeletedFiles(deletedFiles: string[]): Promise<void> {
   ...
   */
  private async updateFileMetadata(currentFiles: FileMetadata[]): Promise<void> {
    ...
    try {
      ...
      if (metadata) {
        ...
        for (const [path, fileData] of Object.entries(data)) {
          this.fileMetadata.set(path, fileData as FileMetadata);
        }
      }
    } catch (error) {
  ...
  private async saveFileMetadata(): Promise<void> {
    const data: Record<string, FileMetadata> = {};
    for (const [path, metadata] of this.fileMetadata.entries()) {
      data[path] = metadata;
    }
  ...
  private async extractTypes(content: string, filePath: string): Promise<any[]> {
  ...
  private async insertFunctions(functions: any[], embeddings: Float32Array[]): Promise<void> {
    const stmt = this.db.prepare(`
  ...
  private async insertTypes(types: any[]): Promise<void> {
    const stmt = this.db.prepare(`
      INSERT INTO functions (
