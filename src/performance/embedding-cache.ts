...
export class EmbeddingCache {
   ...
   */
  private addToCache(key: string, embedding: Float32Array, size: number): void {
    // Evict if necessary
...
export function getEmbeddingCache(maxSizeMB: number = 100): EmbeddingCache {
  if (!globalCache) {
...
export function destroyEmbeddingCache(): void {
  if (globalCache) {
    globalCache.destroy();
    globalCache = null;
  }
