...
import { batchGenerateEmbeddings } from '../embeddings/embeddings.js';
// ─── Enhanced Index Building ─────────────────────────────────────────────────────
export async function buildEnhancedIndex(projectPath) {
...
async function extractFunctionsFromContent(content, filePath, config) {
    const functions = [];
...
function extractFunctionFromMatch(match, lines, filePath) {
...
function extractArrowFunctionFromMatch(match, lines, filePath) {
...
function extractMethodFromMatch(match, lines, filePath) {
...
function extractJSDocInfo(lines, startLine) {
    const jsDocInfo = {
        what: undefined,
        how: undefined,
        why: undefined,
    ...
    if (jsDocText) {
        ...
        // Check completeness
        jsDocInfo.isComplete = !!(jsDocInfo.what && jsDocInfo.how && jsDocInfo.why &&
            jsDocInfo.domain && jsDocInfo.tags && jsDocInfo.sideeffects &&
            jsDocInfo.systemlayer);
    }
    return jsDocInfo;
}
function extractFunctionSignature(lines, lineNumber) {
...
function extractArrowFunctionSignature(lines, lineNumber) {
...
function extractInlineComments(lines, lineNumber) {
    const comments = [];
...
// ─── Type Extraction ─────────────────────────────────────────────────────────
async function extractTypesFromContent(content, filePath, config) {
    const types = [];
    const lines = content.split('\n');
    const relativePath = path.relative(config.projectRoot, filePath);
...
// ─── Database Operations ─────────────────────────────────────────────────────
async function insertEnhancedFunctions(db, functions, embeddings, config) {
...
async function insertEnhancedTypes(db, types, config) {
    const stmt = db.prepare(`
    INSERT INTO functions (
...
//# sourceMappingURL=enhanced-indexer.js.map