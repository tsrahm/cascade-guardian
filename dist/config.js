 ...
 */
export function getGuardianHome() {
 ...
 */
export function detectProjectRoot(fromPath) {
    // 1. Explicit override
 ...
 */
export function detectProjectRootFromFile(filePath) {
...
function detectSourceDirectories(projectRoot) {
        ...
        try {
            ...
            if (tsconfig.include && Array.isArray(tsconfig.include)) {
                ...
                const dirs = tsconfig.include
                    ...
                    .filter((dir) => !dir.startsWith('.') && dir !== '*' && dir !== '**')
                    .filter((dir, i, arr) => arr.indexOf(dir) === i);
                if (dirs.length > 0) {
                    // Verify directories actually exist
                    const existing = dirs.filter((dir) => fs.existsSync(path.join(projectRoot, dir)));
    ...
    const found = candidates.filter(dir => fs.existsSync(path.join(projectRoot, dir)));
    return found.length > 0 ? found : ['.'];
}
/**
 * Auto-detects documentation directories.
 */
function detectDocsDirectories(projectRoot) {
 ...
 */
export function resolveConfig(fromPath) {
    // Detect project root.
 ...
 */
export function registerProject(config) {
