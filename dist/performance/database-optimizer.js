...
export class DatabaseOptimizer {
     ...
     */
    executeOptimizedSearch(query, filters) {
        let sql = `
     ...
     */
    optimizedGetCallers(functionName, filePath) {
     ...
     */
    optimizedGetImpact(functionName, maxDepth = 5) {
    ...
    `;
        ...
        const affectedFunctions = result.map((row) => ({
            name: row.name,
     ...
     */
    batchInsert(table, columns, data) {
        if (data.length === 0)
            return;
...
export function getDatabaseOptimizer(dbPath) {
    if (!globalOptimizer) {
        globalOptimizer = new DatabaseOptimizer(dbPath);
    }
    return globalOptimizer;
}
export function destroyDatabaseOptimizer() {
    if (globalOptimizer) {
